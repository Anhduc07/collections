...
public class StreamExamples {
    ...
    public static void convertListToOther() {
        List<example1> persons = new ArrayList<>();
        persons.add(new example1("DucAnh", 18));
        persons.add(new example1("Nam", 20));
        persons.add(new example1("Hieu", 22));
    ...
    public static void findElement() {
        ...
        Optional<example1> result = persons.stream().filter(p -> p.getName().equals("DucAnh")).findAny();
        if (result.isPresent()) {
    ...
    public static void sumOfElements() {
        int[] intArray = {23,45,76,14,67,108};
